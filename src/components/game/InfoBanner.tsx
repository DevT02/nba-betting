...
const InfoBanner: React.FC<InfoBannerProps> = ({
  ...
  // Animation variants for sliding from the right.
  const variants = {
    open: {
      x: 0,
      opacity: 1,
    ...
    peek: {
      x: 40, // small offset for a peek effect
      opacity: 0.95,
  ...
  const effectiveMode = isHovered ? "open" : mode;
    ...
    <AnimatePresence>
      {effectiveMode !== "hidden" && (
        <motion.div
          ...
          animate={variants[effectiveMode]}
          ...
          className="fixed right-0 max-w-[350px] w-[90%] bg-background p-4 shadow-lg z-40 overflow-auto transition-all border-2 border-border"
